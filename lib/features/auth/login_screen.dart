...
Future<void> _googleSignIn() async {
  ...
  try {
    final pendingToken = await ref.read(authControllerProvider.notifier).googleLogin();
    ...
    if (!mounted) return;
    final message = e is AppError ? e.message : 'Google sign-in failed';
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(message), backgroundColor: kStatusFailed),
    );
  } finally {
                  ...
                  children: [
                    ...
                    Align(
                      ...
                      child: TextButton(
                        ...
                        child: const Text(
                          'Forgot password?',
                          style: TextStyle(color: kUzinduziRed, fontWeight: FontWeight.w600),
                        ),
                      ),
                    ),
                    ...
                    const Row(children: [
                      ...
                      Padding(
                        ...
                        child: Text('or', style: TextStyle(color: kUzinduziGrey, fontSize: 12)),
                      ),
                      Expanded(child: Divider(color: kUzinduziDivider)),
                    ]),
                    const SizedBox(height: 20),
                    ...
                    Row(
                      ...
                      children: [
                        const Text('New here? ', style: TextStyle(color: kUzinduziGrey)),
                        TextButton(
                          onPressed: () => Navigator.of(context).pushNamed(AppRoutes.register),
                          child: const Text(
                            'Create an account',
                            style: TextStyle(color: kUzinduziRed, fontWeight: FontWeight.w700),
                          ),
                        ),
                      ],
                    ),
